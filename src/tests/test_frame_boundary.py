...
pytestmark = pytest.mark.frame_boundary


def test_frame_boundary(sdk_tools, numpy_helper):
    ...
    sdk_tools.run_scenario("test_frame_boundary/frame_boundary.json")

    result = numpy_helper.load("outBufferAdd2.npy", np.float32)
    assert np.array_equal(result, input1 + input2 + input2)


def test_frame_boundary_command_only(sdk_tools, numpy_helper):
    input1 = numpy_helper.generate([10], dtype=np.float32, filename="inBufferA.npy")
    input2 = numpy_helper.generate([10], dtype=np.float32, filename="inBufferB.npy")

    sdk_tools.run_scenario("test_frame_boundary/frame_boundary_command_only.json")


def test_frame_boundary_and_shader(sdk_tools, numpy_helper):
    input1 = numpy_helper.generate([10], dtype=np.float32, filename="inBufferA.npy")
    input2 = numpy_helper.generate([10], dtype=np.float32, filename="inBufferB.npy")

    sdk_tools.compile_shader("test_shader/add_shader.comp", {"TestType": "float"})
    sdk_tools.run_scenario("test_frame_boundary/frame_boundary_and_shader.json")
