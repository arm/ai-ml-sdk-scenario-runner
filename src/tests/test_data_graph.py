...
def test_conv2d_spv(sdk_tools, resources_helper, numpy_helper):
    ...
    sdk_tools.run_scenario("test_data_graph/conv2d.json")
...
def test_data_graph_spv_path_unreadable(sdk_tools, resources_helper, numpy_helper):
...
def test_data_graph_missing_graph_constant(sdk_tools, resources_helper, numpy_helper):
...
def test_data_graph_rejects_glsl_shader(sdk_tools, resources_helper, numpy_helper):
    # Prepare inputs for maxpool
    numpy_helper.generate([1, 16, 16, 16], dtype=np.int8, filename="maxpoolInput.npy")
